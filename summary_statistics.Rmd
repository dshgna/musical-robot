...
```{r}
# Create Corpus
blasphemy_corpus <- corpus(blasphemy_preprocessed_final)
```
...
```{r}
(blasphemy_all_summary <- blasphemy_summary %>%
    summarise(across(where(is.numeric), ~ median(.x, na.rm = TRUE))))
```

```{r}
summary(blasphemy_summary)
```
...
```{r}
blasphemy_summary_no_NA <- blasphemy_summary %>%
  filter(region != 'Not Provided')
st(blasphemy_summary_no_NA, 
   group = 'region', 
   group.test = TRUE, 
   out = 'latex')
```
...
(tstat_col_caps <- blasphemy_tokens %>% 
                  textstat_collocations(min_count = 30, size = 2:3))
```

```{r}
(blasphemy_dfm <- blasphemy_tokens %>%
        tokens_compound(pattern = tstat_col_caps[tstat_col_caps$z > 15]) %>%
        dfm() %>%
...
```{r}
docvars(blasphemy_dfm, 'region') <- corpus_docvars
```
