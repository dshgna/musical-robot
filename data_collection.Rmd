...
```{r}
#mutate(country_from_location = possibly(lookup_coords_nominatim, otherwise = NA_real_)(location)) 
...
blasphemy_preprocessed <- blasphemy_tweets %>%
  ...
  mutate(country_from_location = ifelse(!is.na(country), country, get_country_from_location(location))) %>%
  select(user_id, status_id, created_at, screen_name, text, reply_to_status_id, 
         is_quote, is_retweet, country, location, country_from_location)  
...
blasphemy_preprocessed_final <- blasphemy_preprocessed %>%
  ...
  mutate(country_from_location = ifelse(country_from_location == 'Not provided',
                                        NA,
                                        country_from_location)) %>%
